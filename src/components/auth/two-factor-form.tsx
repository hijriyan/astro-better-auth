...
export function TwoFactorForm() {
  ...
  const verifyTotp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.error) setError(result.error.message || '2FA verification failed');
  ...
  const verifyOtp = async (e: React.FormEvent) => {
    ...
    setError('');
    try {
      const result = await (authClient as any).twoFactor.verifyOtp({ code });
      if (result.error) setError(result.error.message || '2FA verification failed');
      else window.location.href = callbackURL;
    } catch {
      setError('An error occurred');
        ...
        <form onSubmit={verifyTotp} className="space-y-4">
          ...
          {methods.includes('otp') && (
            <Button
              ...
              variant="ghost"
              className="w-full"
              onClick={() => { setView('otp'); setCode(''); setError(''); void sendOtp(); }}
              disabled={loading}
            >
